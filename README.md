...
该项目可以预测银行顾客何时可能会选择不与银行继续进行业务。 它使用Keras和Tensorflow以及Kaggle数据集。
银行的顾客流失可能对业务非常 不利，但该项目可以为您提供一个提前预知客户流失的解决方法。该项目使用神经网络和10,000个顾客信息的数据集来预测该银行的顾客是否要离开银行。
此模型旨在帮助指导人们建立自己的等效网络。它目前尚未准备好进行实地部署，而应该进一步推进个性化和模型完善。
